    ...
    <f7-list inline-labels>
      ...
      <f7-list-item title="标签">
        <div strong style="width: 70%;height: 100%;">
          <f7-chip v-for="(tag,index) in myInfo.tags" :key="index" :text="tag" deleteable
                   @delete="deleteTag(myInfo.tags,index,'标签')"></f7-chip>
          <f7-chip text="添加" color="blue" @click="addTag(myInfo.tags,'标签')"></f7-chip>
        </div>
      </f7-list-item>
      <f7-list-item title="是否关注">
        <f7-toggle slot="after" :checked="myInfo.interest"
                   @change="myInfo.interest = $event.target.checked"></f7-toggle>
      </f7-list-item>
